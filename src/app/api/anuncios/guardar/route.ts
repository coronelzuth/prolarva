...
export async function POST(req: NextRequest) {
  try {
    const { adminCode, texto, activo } = await req.json();
    ...
    await db.from('anuncios').update({ activo: false }).neq('id', 'none');

    if (texto && activo !== false) {
      const id = crypto.randomUUID();
      const { error } = await db.from('anuncios').insert({ id, texto, activo: true });
