...
- ✅ **Botón "Aplicar mis datos" en el panel "Ajustar datos"** — al tocarlo sube al resultado y muestra "✅ Recalculado con tus datos"; se resetea al editar cualquier campo. Estado `ajAplicado`.
- ✅ **El lead ahora guarda los datos del lote** — `precio_bulto`, `bulto_kg`, `dias_ciclo`, `precio_venta`, `mortalidad`, `pct_bsf`, `perdida_anual_cop`, `datos_ajustados`. `/api/leads/guardar` intenta el insert completo y cae al insert base si faltan columnas. `AdminView` muestra un bloque "Datos del productor" en el lead expandido + CSV ampliado. Commit `142d2eb`, deploy `prolarva-nq1m0wgnc` (2026-09-07).
- 🔴 **BUG DESCUBIERTO 2026-09-07 — la captura de leads está rota en producción.** `POST /api/leads/guardar` devuelve `Could not find the 'especie' column of 'leads' in the schema cache`. **Ningún lead de la calculadora se está guardando** (el botón de WhatsApp igual abre, así que de cara al usuario no se nota). No lo causó este cambio — es previo. **Fix: correr `supabase/leads-datos-lote.sql` en Supabase SQL Editor** (hace `ADD COLUMN IF NOT EXISTS` de todas las columnas + `NOTIFY pgrst, 'reload schema'`). Después probar: `POST https://prolarva.co/api/leads/guardar` con `{nombre,whatsapp}` debe responder `{"success":true}`.
