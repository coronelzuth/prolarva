...
- ✅ **`supabase/seguridad-tablas.sql`** *(nuevo — pendiente de correr)* — quita a la anon key TODO acceso a `leads`, `invitaciones`, `ventas`, `password_resets`, `push_subscriptions` (solo se tocan desde API routes con service_role). Antes cualquiera con la anon key podía leer la lista de leads con WhatsApp, los códigos de invitación sin usar, y los tokens de recuperación de contraseña.
- 📌 **PENDIENTE (Juliana):** correr **`supabase/seguridad-tablas.sql`** en el SQL Editor. No rompe nada (esas rutas ya usan service_role). Pásame el resultado de la verificación.
